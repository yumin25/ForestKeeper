...
public class MountainRepositorySupport {
    ...
    public List<Tuple> findByDistance(double lat, double lng) {
        ...
        NumberExpression<Double> dist =
                MathExpressions.sin(Expressions.constant(lat * Math.PI / 180.0))
                        ...
                        .add(MathExpressions.cos(Expressions.constant(lat * Math.PI / 180.0))
                                ...
                                .multiply(MathExpressions.cos(qMountain.lng.subtract(Expressions.constant(lng)).multiply(Math.PI).divide(180.0)))
                        );
    ...
    public List<Tuple> findByHeight(double avg) {
        ...
        return jpaQueryFactory.select(dif, qMountain)
                .from(qMountain)
                .orderBy(dif.asc())
                .limit(5)
                .fetch();
